...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if (!strcasecmp(pszExt, ".k7")) {
      return FMGR_FORMAT_K7;
    } else 
    if (!strcasecmp(pszExt, ".sap")) {
      return FMGR_FORMAT_SAP;
    } else 
  ...
  while(nfiles<PSP_FMGR_MAX_ENTRY){
    ...
    if ( (format == user_file_format            ) || 
         ((format == FMGR_FORMAT_ZIP          ) && 
          ...
          (user_file_format != FMGR_FORMAT_SAP)) ) 
    {
      nfiles++;
    }
  }
...
psp_fmgr_menu(int format, int disk_id)
{
  ...
  if (format == FMGR_FORMAT_SAP) user_filedir = user_filedir_disk;
  else
  if (format == FMGR_FORMAT_STATE) user_filedir = user_filedir_snap;
  else                             user_filedir = user_filedir_k7;
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      {
        if (file_format == FMGR_FORMAT_STATE) /* State */ error = thom_load_state(user_filename);
        else
        if (file_format == FMGR_FORMAT_K7) /* K7 */ error = thom_load_k7(user_filename);
        else
        if (file_format == FMGR_FORMAT_SAP) /* SAP */ error = thom_load_sap(user_filename, disk_id);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = thom_load_file_settings(user_filename);
      }
    }
