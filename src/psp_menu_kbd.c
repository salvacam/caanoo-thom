...
psp_display_screen_kbd_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++) 
  {
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) thom_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) thom_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) thom_key = loc_kbd_mapping_R[kbd_id];

      if ((thom_key >= 0) && (thom_key < THOMK_MAX_KEY)) {
        strcpy(buffer, psp_thom_key_info[thom_key].name);
      } else 
      if (thom_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (thom_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (thom_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", thom_key);
      }
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = THOMK_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= THOMK_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
      ...
      if (danzeff_key > DANZEFF_START) {
        ...
        if (thom_key != -1) {
              ...
              (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
          {
            ...
            if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = thom_key;
            else
            if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = thom_key;
            else
            if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = thom_key;
          }
        }
