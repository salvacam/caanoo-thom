...
#define SDL_SOUND_BUFFER_BYTES  (SDL_SOUND_BUFFER_SIZE * 4)
# endif


static int audio_fd;
static unsigned char sound_buffer[SOUND_BUFFER_SIZE];
static unsigned char sdl_sound_buffer[SDL_SOUND_BUFFER_BYTES];
...
{
  if ((! THOM.thom_sound_mute) && THOM.thom_snd_enable)  {
    if (len > SDL_SOUND_BUFFER_BYTES) len = SDL_SOUND_BUFFER_BYTES;
...
# endif
  ...
  // convert 8bits 22050 to 2x16bits 44100
  ushort* scan_buffer = (ushort *)sdl_sound_buffer;
  for (i = 0; i < SDL_SOUND_BUFFER_SIZE; i++) {
   ushort v = sound_buffer[i >> 1] << 8;
   *scan_buffer++ = v;
   *scan_buffer++ = v;
  }
