...
psp_sdl_display_splash()
{
  ...
  int x = (320 - ((strlen(THOM_VERSION) + 2) * 6));
  int y = 2;
  int col = psp_sdl_rgb(0xff, 0xff, 0xff);

  psp_sdl_blit_splash();
  psp_sdl_print(x, y, THOM_VERSION, col);
  psp_sdl_flip();
...
{
  /* First dump vram to my_surface */
  int x;
  int y;
  u16* src_pixel = (u16*)to7_vram;
  u16* dst_pixel = (u16*)my_surface->pixels;
  u16* scan_src_pixel = 0;

  for (y = 0; y < SNAP_HEIGHT; y++) {
    scan_src_pixel = src_pixel + (TO7_SCREEN_W * y * 2);
    for (x = 0; x < SNAP_WIDTH; x++) {
      *dst_pixel++ = scan_src_pixel[x * 2];
    }
  }
...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", THOM.thom_home_dir, THOM.psp_screenshot_id++);
  if (THOM.psp_screenshot_id >= 10) THOM.psp_screenshot_id = 0;
  psp_sdl_save_png(back_surface, TmpFileName);
}
...
# endif
  ...
  for (Index = 0; Index < THOM_MAX_SAVE_STATE; Index++) {
    THOM.thom_save_state[Index].surface = 
